...
class Routers{
    ...
    public async get(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url);
        if(this.NMidllewareList ==[]){
            if(ParaMidlew == []){
                // 이해안가는 부분 
                // ParaMidlew 비어 있는데 Middleware에 넘긴다>?
    ...
    public async post(url:string, callback:((context:any)=>void), ParaMidlew:Array<any>=[]){
        let uris = this.getUri(url);
    ...
    public async put(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url)
        if (this.NMidllewareList == [] ) {
            if (ParaMidlew == []) {
    ...
    public async delete(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url)
        if (this.NMidllewareList == [] ) {
            if (ParaMidlew == []) {
    ...
    public async options(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url)
        if (this.NMidllewareList == [] ) {
            if (ParaMidlew == []) {
    ...
    public async patch(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url)
        if (this.NMidllewareList == [] ) {
            if (ParaMidlew == []) {
    ...
    public async head(url: string, callback: ((context: any) => void), ParaMidlew: Array<any> = []) {
        let uris = this.getUri(url)
        if (this.NMidllewareList == [] ) {
            if (ParaMidlew == []) {
    ...
    public Lmatch(g:any,uri:any,callback:((context :any)=>void),ParaMidlew: Array<any> = []) {
        if (g == 'get') {
    ...
    public match(match:Array<string> ,uri:string ,callback:((context :any)=>void), ParaMidlew: Array<any> = []) {
        for (const key in match) {
            this.Lmatch(match[key],uri,callback,ParaMidlew)
        }
    }
    public any(uri:any,callback:((context :any)=>void),ParaMidlew: Array<any> = []){
